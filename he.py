...
if __name__ == "__main__":
    ...
    compute_hist[grid_size, block_size](hsv_gpu, hist_gpu)
    ...
    compute_cdf[cdf_grid_size, cdf_block_size](hist_gpu, cdf_gpu, hist_sum)
    # cdf = (np.cumsum(hist).astype('float32') / hist.sum()) * 255
    # cdf = np.round(cdf).astype('uint8')
    # cdf_gpu = cuda.to_device(cdf)
    equalize_hist[grid_size, block_size](hsv_gpu, cdf_gpu, he_gpu)
